...
class CostFrame:
    ...
    The orientation is described with angular vector w
    """
    ...
    def calc(self, q):
        ...
        self.w = mat2w(self.ori)
        self.r_pos = self.sel_vector[:3]*(self.pos-self.desired_pose[:3]) 
        self.r_ori = self.sel_vector[3:]*(self.w-self.desired_pose[3:])
        self.r = np.concatenate([self.r_pos, self.r_ori])
